...
export function AppSidebar({ role, menuItems }: AppSidebarProps) {
      ...
      return (
        <div key={item.id}>
          {hasChildren ? (
            ...
            <Link href={item.href!} className={sharedItemClass}>
              {ActiveIndicator}
              <Icon className="w-[17px] h-[17px] flex-shrink-0" />
              {!collapsed && (
  ...
  // Split brand name into main + highlighted suffix
    ...
    <TooltipProvider delayDuration={0}>
      <style>{`
        ...
        .sidebar-nav-scroll::-webkit-scrollbar-thumb {
          background: rgba(14,165,233,0.25); border-radius: 99px;
        }
        .sidebar-nav-scroll::-webkit-scrollbar-thumb:hover {
          background: rgba(14,165,233,0.55);
        }
        .sidebar-nav-scroll { scrollbar-width: thin; scrollbar-color: rgba(14,165,233,0.25) transparent; }
      `}</style>
          ...
          >
            ...
            >
              ...
              >
                {brandMain}
                {highlight && (
                  <span style={{ color: role.accentColor ?? '#90EE90' }}>
                    {highlight}
                  </span>
                )}
              </span>
              <span
                className="text-[0.58rem] uppercase tracking-[1.8px] font-medium whitespace-nowrap"
                style={{ color: role.accentColor ?? '#90EE90' }}
              >
                {role.roleLabel}
              </span>
            </div>
        ...
        >
          {!collapsed && user && (
            <div className="mb-2.5 px-3 py-2 rounded-[10px] bg-[rgba(255,255,255,0.04)] border border-[rgba(14,165,233,0.1)]">
              <p
                className="text-[0.85rem] font-semibold text-[#e0f2fe] truncate"
                style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
              >
                {user.name}
              </p>
              <p
                className="text-[0.68rem] uppercase tracking-[0.5px] font-medium"
