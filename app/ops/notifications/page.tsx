...
const NOTIF_TYPE_META: Record<string, { icon: any; color: string; bg: string; border: string }> = {
  ...
  driver_assigned: { icon: Truck, color: 'text-cyan-400', bg: 'bg-cyan-500/10', border: 'border-cyan-500/20' },
  maintenance_due: { icon: Wrench, color: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/20' },
  driver_off_duty: { icon: Clock, color: 'text-muted-foreground', bg: 'bg-muted/40', border: 'border-border/40' },
};
...
export default function NotificationsPage() {
  ...
  const stats = useMemo(() => ({
    ...
    critical: notifications.filter(n => n.type === 'shipment_delayed' || n.type === 'payment_overdue' || n.type === 'maintenance_due').length,
  }), [notifications]);
  ...
  return (
    <PageWrapper
      title="Notifications"
      description="Operational alerts and notifications"
      actions={
        <div className="flex items-center gap-2">
          {stats.unread > 0 && (
            <Badge variant="default" className="text-[0.65rem] px-2 py-0.5 h-auto bg-primary/15 text-primary border-primary/20 gap-1">
              <Bell className="w-3 h-3" /> {stats.unread} unread
            </Badge>
          )}
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <KPICard title="Total Notifications" value={stats.total} icon={<Bell className="w-5 h-5" />} iconColor="indigo" />
        <KPICard title="Unread" value={stats.unread} icon={<Clock className="w-5 h-5" />} iconColor="amber" />
        <KPICard title="Requires Action" value={stats.critical} icon={<AlertCircle className="w-5 h-5" />} iconColor="red" />
      </div>
      ...
      <Card className="bg-card border border-border/60 shadow-soft mb-6">
        <CardContent className="p-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {filtered.length > 0 && (
              <span className="text-[0.65rem] text-muted-foreground ml-auto">{filtered.length} notification(s)</span>
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <div className="space-y-3">
        {filtered.length > 0 ? (
          filtered.map(notif => {
            const meta = NOTIF_TYPE_META[notif.type] || { icon: Bell, color: 'text-muted-foreground', bg: 'bg-muted/40', border: 'border-border/40' };
            const Icon = meta.icon;
            return (
              <div key={notif.id}
                className={`group bg-card border border-border/60 rounded-xl shadow-soft overflow-hidden transition-all duration-300 hover:border-primary/25 hover:shadow-[0_8px_32px_oklch(var(--primary)/0.08)] ${!notif.read ? 'border-l-2 border-l-primary' : ''}`}
              >
                <div className="p-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            <h3 className="text-[0.82rem] font-bold font-display text-foreground">{notif.title}</h3>
                            <span className={`px-1.5 py-0.5 rounded text-[0.55rem] font-bold border ${meta.bg} ${meta.color} ${meta.border}`}>
                              {notif.type.replace(/_/g, ' ')}
                            </span>
                          </div>
                          <p className="text-[0.72rem] text-muted-foreground mt-0.5">{notif.message}</p>
                        </div>
                      ...
                      <div className="flex items-center gap-3 mt-2.5">
                        ...
                        {notif.actionUrl && (
                          <a href={notif.actionUrl}
                            className="text-[0.6rem] font-semibold text-primary hover:text-primary/80 transition-colors flex items-center gap-0.5">
