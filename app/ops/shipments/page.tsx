...
import {
  Search, X, RotateCcw, Package, Truck, Clock, CheckCircle2,
  AlertTriangle, MapPin, ArrowRight, Plus,
} from 'lucide-react';
...
export default function OpsShipmentsPage() {
  ...
  useEffect(() => {
    shipmentService.list({ role: 'Dispatcher' }).then((data) => {
      setShipments(data);
      setLoading(false);
    });
  }, []);
      ...
      render: (s) => (
        <div className="min-w-0">
          <div className="flex items-center gap-1.5 text-xs">
            <span className="truncate max-w-[80px]">{s.route.origin}</span>
            <ArrowRight className="w-2.5 h-2.5 text-muted-foreground/40 flex-shrink-0" />
            <span className="truncate max-w-[80px]">{s.route.destination}</span>
          </div>
  ...
  if (loading) {
    return (
      <PageWrapper title="Shipments" description="View and manage all operational shipments">
  ...
  return (
    <PageWrapper
      title="Shipments"
      description="View and manage all operational shipments"
      actions={
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <KPICard title="Total Active" value={stats.total} icon={<Package className="w-5 h-5" />} iconColor="indigo" />
        ...
        <KPICard title="Out for Delivery" value={stats.outForDelivery} icon={<CheckCircle2 className="w-5 h-5" />} iconColor="teal" />
      </div>
