...
import { Search, X, RotateCcw, Truck, Package, ShieldAlert, CheckCircle2, Clock, AlertTriangle, ArrowRight, MapPin, User, Phone } from 'lucide-react';
import type { ConsolidatedShipment } from '@/types/shipment';

const DISPATCH_STATUSES = ['In Transit', 'Out for Delivery', 'Picked Up'];
...
export default function AuditDispatchesPage() {
  ...
  useEffect(() => {
    shipmentService.list({ role: 'AuditorReadOnly' }).then((data) => {
      setShipments(data.filter(s => DISPATCH_STATUSES.includes(s.status)));
      setLoading(false);
    }).catch((err) => {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter(s =>
        ...
        s.assignedDriver?.toLowerCase().includes(q) ||
  ...
  const columns: Column<ConsolidatedShipment>[] = [
      ...
      render: (s) => (
        <div className="min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-medium text-foreground truncate max-w-[120px]">{s.sender.name}</span>
          </div>
          <div className="flex items-center gap-1 mt-0.5">
            <Phone className="w-2 h-2 text-muted-foreground/40" />
            <span className="text-[0.6rem] text-muted-foreground truncate">{s.sender.phone}</span>
          </div>
        </div>
      ),
    },
    {
      ...
      render: (s) => (
        <div className="min-w-0">
          <div className="flex items-center gap-1.5">
            <span className="text-xs font-medium text-foreground truncate max-w-[100px]">{s.route.origin}</span>
            <ArrowRight className="w-2.5 h-2.5 text-muted-foreground/40" />
            <span className="text-xs font-medium text-foreground truncate max-w-[100px]">{s.route.destination}</span>
          </div>
          <div className="flex items-center gap-1 mt-0.5">
  ...
  if (loading) {
    return (
      <PageWrapper title="Audit: Dispatches" description="Read-only view of active dispatch records">
        <LoadingState rows={8} message="Loading dispatch records..." />
      </PageWrapper>
    );
  }
  ...
  return (
    <PageWrapper
      title="Audit: Dispatches"
      description="Read-only view of active and in-progress dispatch records"
      actions={
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <KPICard title="Total Dispatches" value={stats.total} icon={<Truck className="w-5 h-5" />} iconColor="indigo" />
        <KPICard title="In Transit" value={stats.inTransit} icon={<Clock className="w-5 h-5" />} iconColor="indigo" />
        <KPICard title="Out for Delivery" value={stats.outForDelivery} icon={<Truck className="w-5 h-5" />} iconColor="amber" />
        <KPICard title="Picked Up" value={stats.pickedUp} icon={<Package className="w-5 h-5" />} iconColor="green" />
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by tracking ID, customer, driver, sender, receiver, or route..."
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description="No active dispatch records match your current search or filter criteria."
          action={
