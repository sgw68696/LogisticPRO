...
import { mockConsolidatedShipments, getShipmentsForRole } from '@/data/shipments';
...
function applyFilters(shipments: ConsolidatedShipment[], filters?: ShipmentFilters): ConsolidatedShipment[] {
  if (!filters) return shipments;
  let result = [...shipments];

  if (filters.role) result = getShipmentsForRole(filters.role, result);
...
export const shipmentService = {
  async list(filters?: ShipmentFilters): Promise<ConsolidatedShipment[]> {
    if (APP_CONFIG.USE_MOCK) {
      await new Promise(r => setTimeout(r, 300));
      return applyFilters(mockConsolidatedShipments, filters);
    }
  ...
  async listPaginated(filters?: ShipmentFilters): Promise<PaginatedResult<ConsolidatedShipment>> {
    const page = filters?.page || 1;
    const pageSize = filters?.pageSize || 20;
    if (APP_CONFIG.USE_MOCK) {
      await new Promise(r => setTimeout(r, 300));
      const filtered = applyFilters(mockConsolidatedShipments, filters);
  ...
  async getById(id: string): Promise<ConsolidatedShipment | null> {
    if (APP_CONFIG.USE_MOCK) {
  ...
  async create(data: Partial<ConsolidatedShipment>): Promise<ConsolidatedShipment> {
    if (APP_CONFIG.USE_MOCK) {
      await new Promise(r => setTimeout(r, 400));
      const shipment: ConsolidatedShipment = {
        id: `csh-${String(mockConsolidatedShipments.length + 1).padStart(3, '0')}`,
        trackingNumber: `CSH-2026-${String(10001 + mockConsolidatedShipments.length).padStart(5, '0')}`,
        status: 'Pending',
  ...
  async update(id: string, updates: Partial<ConsolidatedShipment>): Promise<ConsolidatedShipment> {
    if (APP_CONFIG.USE_MOCK) {
      await new Promise(r => setTimeout(r, 300));
  ...
  async updateStatus(id: string, status: ShipmentStatus, notes?: string): Promise<ConsolidatedShipment> {
    if (APP_CONFIG.USE_MOCK) {
      await new Promise(r => setTimeout(r, 300));
      const idx = mockConsolidatedShipments.findIndex(s => s.id === id);
      if (idx === -1) throw new Error('Shipment not found');
  ...
  async bulkUpdateStatus(ids: string[], status: ShipmentStatus): Promise<void> {
    if (APP_CONFIG.USE_MOCK) {
